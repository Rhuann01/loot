...
export function getMelhoresDescontos(limite: number) {
  return db.prepare(`
...
export function getProdutosSelecionadosManualmente() {
  return db.prepare(`
...
export function alternarSelecao(id: number, selecionado: boolean) {
  db.prepare(`UPDATE produtos SET selecionado = ? WHERE id = ?`)
    .run(selecionado ? 1 : 0, id)
}


export function getConfig(chave: string): string | undefined {
  const linha = db.prepare(`SELECT valor FROM config WHERE chave = ?`).get(chave) as
    | { valor: string }
    | undefined
  return linha?.valor
}
...
export function listarCategoriasMonitoradas() {
  return db.prepare(`SELECT * FROM categorias_monitoradas WHERE ativa = 1`).all()
}

export function adicionarCategoriaMonitorada(categoria_id: string, categoria_nome: string) {
  db.prepare(`
